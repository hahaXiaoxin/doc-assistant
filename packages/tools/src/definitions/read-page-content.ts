 ...
 *   `{ content, hasMore, nextOffset?, totalChars, title, extractor, charCount }`。
...
export interface ReadPageContentResult {
  ...
  extractor: string;
  /** 提取器自身汇报的 charCount（与 totalChars 一致，保留向后兼容） */
  charCount: number;
}
...
export const readPageContentTool: ToolDefinition<ReadPageContentArgs, object> = {
  ...
  description:
    [
      ...
      '返回字段：content（本段正文）、hasMore（是否还有剩余）、nextOffset（hasMore=true 时给出的下一段起始位置）、totalChars（正文总字符数）、title、extractor、charCount。',
