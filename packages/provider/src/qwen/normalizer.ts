 ...
 * - 处理千问特定字段 `reasoning_content`：AI SDK 在 reasoning 支持的 provider 上会发出 'reasoning'
 *   类型的 part，我们映射为 `{ type: 'reasoning-delta', delta }`。
 *
 * 约束：
...
const logger = createLogger('provider:qwen:normalizer');
...
export function normalizeStreamPart(part: UnknownStreamPart): ChatChunk[] {
  switch (part.type) {
    ...
    // AI SDK v4 的 reasoning part（对应千问 reasoning_content）
    case 'reasoning':
