...
| **DeepSeek**（`https://api.deepseek.com`） | ✅ | ✅ | — | `deepseek-v4-flash`（低成本快响应）/ `deepseek-v4-pro`（主力档），均为 **1M ctx / 384K out**。若上游自发返回 `reasoning_content`，UI 仍走 `ThinkingBlock` 折叠展示；官方无 embedding 服务 |
...
- Agent 层禁止直接构造 HTTP 请求或绕过 `LLMProvider` 接口接入第三方 SDK;v0.6.0-beta.2
  起 chat 链路在 Provider 内部以裸 fetch + 自己解析 OpenAI SSE 实现,不再依赖
